...
class ModelType extends AbstractType
{
    ...
    public function buildForm(FormBuilderInterface $builder, array $options)
    {
        ...
        foreach ($this->types as $type) {
            ...
            $name = ucfirst($builder->getName());
            $label = (count($this->types) > 1) ? "$name $pluralType" : $name;
        ...
        $builder->addViewTransformer(new ModelTransformer($this->types));

        // True if the client provided the recipient usernames instead of IDs
        // (to support non-JS browsers)
        //
        // $builder->add('ListNames', 'hidden', array(
        //     'attr' => array(
        //         'class' => 'model-select-type',
        //     ),
        //     'data' => true,
        // ));
     ...
     * @param array         $options
     */
    public function finishesView(FormView $view, FormInterface $form, array $options)
    {
        ...
        // Reset listNames in case the user has turned javascript off since
        // the last request
        $view->children['ListNames']->vars['value'] = '1';
    }
    ...
    {
        $data = $event->getData();
        $this->listNames = (bool) $data['ListNames'];
